...
int main(int argc, char *argv[])
{
    ...
    QAction *actShow = trayMenu->addAction(QIcon(":/icons/dashboard.svg"), "显示/隐藏");
    QAction *actSettings = trayMenu->addAction(QIcon(":/icons/settings.svg"), "设置");
    QAction *actEditor = trayMenu->addAction(QIcon(":/icons/edit.svg"), "课表编辑器");
    QMenu *rescheduleMenu = trayMenu->addMenu("调休日");
    rescheduleMenu->setIcon(QIcon(":/icons/schedule.svg"));
    ...
    auto loadQmlWindow = [&engine](const QString &qmlUrl) {
        QQmlComponent comp(&engine, QUrl(qmlUrl));
        if (comp.isError()) {
            QString err = comp.errorString();
            qWarning() << "Failed to load" << qmlUrl << err;
            QMessageBox::critical(nullptr, "QML 加载错误",
        ...
        if (!obj) {
            ...
            QStringList details;
            for (const QQmlError &e : errors) details << e.toString();
            QString err = details.join("\n\n");
            qWarning() << "Failed to create" << qmlUrl << err;
            QMessageBox::critical(nullptr, "QML 创建错误",
        ...
        if (win) {
            ...
            qWarning() << "Loaded QML is not a Window:" << qmlUrl;
            QMessageBox::warning(nullptr, "QML 类型错误",
                                 QString("Loaded QML is not a Window: %1").arg(qmlUrl));
            // if it's not a window, delete the object to avoid leaks
            obj->deleteLater();
        }
    };

    QObject::connect(actSettings, &QAction::triggered, &csesParser, [&]() {
        loadQmlWindow("qrc:/ClassBoard/SettingsDialog.qml");
    });

    QObject::connect(actEditor, &QAction::triggered, &csesParser, [&]() {
        loadQmlWindow("qrc:/ClassBoard/ScheduleEditor.qml");
    });
    ...
    QAction *actSwap = trayMenu->addAction(QIcon(":/icons/swap.svg"), "换课");
    QObject::connect(actSwap, &QAction::triggered, &csesParser, [&]() {
        loadQmlWindow("qrc:/ClassBoard/ClassSwapDialog.qml");
    });
